...
Comment = "Selective coating based on W and Al2O3, made of 3 cermet layers and 2 antireflective layers" # Comment to be written in the simulation text file
Mat_Stack = ["Fe", "air-SiO2"]
...
Wl = Wl_selectif() #np.arange(320 , 2505, 5) # /!\ Last value is not included in the array
...
vf_range = (0.01 , 0.02) #  volumic fraction of inclusion in host matrix, must be included in (0,1)
...
nb_generation = 2 # Number of generation/iteration. For DEvol is also used to calculate the budget (nb_generation * pop_size)
...
def run_problem_solution(i):
    ...
    # Run the optimi    sation process (algo), with an evaluate method, a selection method and the parameters dictionary.
