...
evaluate = evaluate_rh# Callable. Name of the cost function
# %% Important parameters 
# Wavelength domain, here from 320 to 2500 nm with a 5 nm step. Can be change!   
Wl = Wl_selectif() # np.arange(320 , 2505, 5) # /!\ Last value is not include in the array
...
d_Stack_Opt = [] # Allows fixing the thickness of a layer that will not be optimized. Set to "no" to leave it unset. For example, if there are three layers, it can be written [,40,]. The code understands that only the middle layer is fixed
# Open the solar spectrum 
Wl_Sol , Sol_Spec , name_Sol_Spec = open_SolSpec('Materials/SolSpec.txt','DC')
...
nb_generation = 30 # Number of generation/iteration. For DEvol is also used to calculate the budget (nb_generation * pop_size)
...
if __name__=="__main__":
    ...
    # =============================================================================
        ...
        plt.legend()
        plt.ylabel("Value of refractive index (-)")
    ...
    if language == "fr":            
        print("L'indice de réfraction de chaque matérial a été écrit dans un fichier à leurs noms")
    if language == "en":            
        print("The refractive index of each material has been written in a file with their names")
